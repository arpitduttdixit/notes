...
async function init() {
  ...
  try {
    ...
    showStatus(
      "Error: Make sure the notes server is running (npm start)",
      "error"
    );
  }
}
...
function displayResults(results) {
  ...
  results.forEach((note) => {
    ...
    item.innerHTML = `<div class="result-title">${escapeHtml(
      note.title
    )}</div>`;
...
async function openNote(note) {
  ...
  try {
    const response = await fetch(
      `${NOTES_SERVER}/notes/${encodeURIComponent(note.id)}`
    );
    const data = await response.json();
    ...
    if (data.body && data.body !== "undefined") {
      ...
      noteBody.innerHTML =
        '<div class="no-results">This note appears to be empty</div>';
    }
  } catch (error) {
...
async function sendToClaude() {
  try {
      ...
      chrome.tabs.sendMessage(
        ...
        (response) => {
          ...
          if (images.length > 0) {
            ...
            setTimeout(() => {
              showStatus(
                `Note sent with ${images.length} image(s)!`,
                "success"
              );
            ...
            chrome.tabs.sendMessage(
              ...
              (response) => {
                ...
                if (images.length > 0) {
                  ...
                  showStatus(
                    `Opening Claude with ${images.length} image(s)...`,
                    "success"
                  );
                } else {
    ...
    sendToClaudeBtn.textContent = "Explain with Claude";
  }
}
...
async function captureChat() {
  ...
  const timeout = setTimeout(() => {
    showStatus(
      "Error: Content script not responding. Try refreshing the Claude page.",
      "error"
    );
  }, 3000);
  ...
  chrome.tabs.sendMessage(tab.id, { type: "extractChat" }, (response) => {
    ...
    if (chrome.runtime.lastError) {
      ...
      showStatus(
        "Error: Content script not loaded. Refresh the Claude page and try again.",
        "error"
      );
      return;
    }
    ...
    if (!response?.success) {
      ...
      showStatus(
        `Error: ${response?.error || "Could not extract chat"}`,
        "error"
      );
      return;
    }
...
function updateSelectionInfo() {
  ...
  selectionInfoEl.textContent = `${count} message${
    count !== 1 ? "s" : ""
  } selected`;
}
...
function formatChatNote(chatData, mode) {
  ...
  switch (mode) {
    ...
    case "code":
      ...
      if (
        lastAssistant &&
        !codeMessages.find((m) => m.index === lastAssistant.index)
      ) {
